...
s32 nuPakMenu(void* frameBuffer, void* fontBuffer)
{
    ...
    nuGfxDisplayOff(); // HM64: Added this.
    ...
    bzero(0x8038f800, 0x70800);    // HM64: Added this.
    nuGfxRetraceWait(1);           // HM64: Added this.
    ...
    for (i = 0; i < 16; i ++){
	pakFile.file_no = i;
    nuContPakFileState(&pakFile, pfs_stat + i); //HM64: Changed this. pfs_stat_ret[i] = nuContPakFileState(&pakFile, pfs_stat + i); 
...
//#endif
    ...
    canvas_cursor( 0xF2, MENU_FREE_BLK_Y1 ); // HM64: Changed this. canvas_cursor(MENU_FREE_BLK_X1, MENU_FREE_BLK_Y1 ); 
	canvas_putstr( "\x1F\x2B\x1E\x1E\x01\x29\x1A\x20\x1E\x2C\x3F" , MENU_COLOR_ITEM, MENU_COLOR_SHADOW ); // HM64: Changed this. canvas_putstr(STR_BLK_LEFT, MENU_COLOR_ITEM, MENU_COLOR_SHADOW);
...
FUNC_END:
    ...
    //nuGfxRetraceWait(1);     HM64: Removed this
